...
def generate_market_growth():
    ...
    for bar in bars:
        yval = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2, yval + 0.3, f'${yval}B', ha='center', va='bottom', fontsize=14, fontweight='bold')
    
    ax.set_ylabel('Market Size (Billions USD)', fontsize=12, fontweight='bold')
    ax.set_title('Graph 3: Explosive Market Growth\n(Weather Derivatives)', fontsize=16, fontweight='bold', pad=20)
