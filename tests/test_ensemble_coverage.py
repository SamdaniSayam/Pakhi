...
class DummyModel1D:
    def predict(self, X):
        return ForecastResult(deterministic=np.ones(len(X)), quantiles={}, skill_scores={}, metadata={})
...
class DummyModelNoQuantiles:
    def predict(self, X):
        return ForecastResult(deterministic=np.ones((len(X), 1)), quantiles={}, skill_scores={}, metadata={})
...
def test_ensemble_retrain_weights_zero_sum():
    ...
    ens.retrain_weights(np.zeros((3, 2)), np.zeros(3), recent_skill=[{"rmse": 1000}, {"rmse": 1000}])
