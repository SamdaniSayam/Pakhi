...
class TestERA5Coverage:
    ...
    def test_get_cds_client_env_key(self):
        ...
        with patch.dict(sys.modules, {"cdsapi": mock_cdsapi}):
            with patch.dict("os.environ", {"CDS_KEY": "test-key-123"}):
    ...
    def test_get_cds_client_no_key_no_rc(self):
        ...
        mock_cdsapi = MagicMock()
        with patch.dict(sys.modules, {"cdsapi": mock_cdsapi}):
            with patch.dict("os.environ", {"CDS_KEY": ""}):
    ...
    def test_fetch_single_level_download_error(self):
        ...
        with patch.object(conn, "_download_dataset", side_effect=RuntimeError("download failed")):
...
class TestSatelliteCoverage:
    ...
    def test_find_latest_file_with_nc(self):
        conn = self._make_connector()
        with patch.object(conn, "_list_s3_objects", return_value=["data/file.nc", "data/other.txt"]):
    ...
    def test_fetch_success(self):
        ...
        with patch.object(conn, "_find_latest_file", return_value="data/file.nc"):
            with patch.object(conn, "_download_s3_file", return_value=tmp):
                result = conn.latest()
                assert result is not None
    ...
    def test_fetch_no_data(self):
        ...
        with patch.object(conn, "_find_latest_file", return_value=None):
            with pytest.raises(RuntimeError, match="No GOES data"):
                conn.latest()
...
class TestMeteostatCoverage:
    ...
    def test_rate_limit_retry(self):
        ...
        ok_resp = {"data": [{"id": "123", "name": "Test", "lat": 40.0, "lon": -74.0,
                              "distance": 5.0, "first": "2020-01-01", "last": "2024-01-01",
                              "hourly": True, "daily": True}]}
        conn._session.get.return_value = MagicMock(
            json=MagicMock(side_effect=[rate_resp, ok_resp]),
    ...
    def test_stations_near_library_import_error(self):
        from pakhi.src.meteostat import MeteostatConnector
        conn = MeteostatConnector.__new__(MeteostatConnector)
        with patch.dict(sys.modules, {"meteostat": None}):
            with pytest.raises(ImportError, match="meteostat"):
...
class TestYahooCoverage:
    ...
    def test_spread_missing_ticker(self):
        ...
        conn.tickers = {"AAPL": "Apple", "MSFT": "Microsoft"}
        conn._yf = MagicMock()
        with patch.object(conn, "history", return_value={"AAPL": pd.DataFrame()}):
    ...
    def test_plot_track_no_cartopy(self):
        ...
        try:
            ...
            result = maps.plot_track(
                track_lats, track_lons, cone_lats, cone_lons,
                title="Test Track"
            )
            assert result is not None
        finally:
...
class TestEnsembleCoverage:
    ...
    def _make_model_stub(self, pred_shape=(1, 2), proba_quantiles=None):
        ...
        if proba_quantiles is not None:
            stub.predict_proba.return_value = MagicMock(deterministic=np.ones(pred_shape),
                                                         quantiles=proba_quantiles)
        else:
            stub.predict_proba.return_value = MagicMock(deterministic=np.ones(pred_shape),
                                                         quantiles={})
        return stub
    ...
    def test_predict_proba_missing_quantile(self):
        from pakhi.models.ensemble import EnsembleForecaster
        m1 = self._make_model_stub(pred_shape=(1, 1),
                                     proba_quantiles={"q0.9": np.ones((1, 1))})
        ef = EnsembleForecaster.__new__(EnsembleForecaster)
        ef.models = [m1]
        ef.method = "mean"
        ef._weights = None
...
class TestLSTMCoverage:
    ...
    def test_export_onnx_success(self):
        import torch
        from pakhi.models.lstm import LSTMForecaster
        f = LSTMForecaster(input_dim=5, hidden_dim=32, n_layers=1,
                           forecast_horizon=1, seq_len=10, dropout=0.1)
        f._init_net()
        f._fitted = True
    ...
    def test_predict_empty(self):
        import torch
        from pakhi.models.lstm import LSTMForecaster
        f = LSTMForecaster(input_dim=3, hidden_dim=16, n_layers=1,
                           forecast_horizon=1, seq_len=5, dropout=0.0,
                           mc_samples=1, quantiles=[0.5], batch_size=32)
        f._init_net()
        X = np.ones((20, 3))
...
class TestDashboardCoverage:
    ...
    def test_signal_style_long(self):
        ...
        signals = [{"instrument": "NG", "action": "LONG", "confidence": 0.8, "size": 0.5, "reasoning": "warm"}]
    ...
    def test_signal_style_short(self):
        ...
        d._use_rich = True
        d._console = MagicMock()
        signals = [{"instrument": "NG", "action": "SHORT", "confidence": 0.8, "size": 0.5, "reasoning": "cold"}]
        d.display_signal_status(signals)
        d._console.print.assert_called_once()
    ...
    def test_signal_style_flat(self):
        ...
        d._use_rich = True
        d._console = MagicMock()
        signals = [{"instrument": "NG", "action": "FLAT", "confidence": 0.3, "size": 0.0, "reasoning": "unclear"}]
        d.display_signal_status(signals)
        d._console.print.assert_called_once()
...
class TestStreamCoverage:
    ...
    def test_process_zarr_dir(self):
        ...
        with patch("pakhi.pipeline.stream.xr.open_zarr", return_value=mock_ds):
    ...
    def test_process_lazy_no_chunks(self):
        ...
        tmp = Path(tempfile.mkdtemp()) / "test.nc"
        tmp.write_bytes(b"fake")
        with patch.object(sp, "process_chunks", return_value=iter([])):
...
class TestBacktestCoverage:
    ...
    def test_backtest_trade_executed(self):
        ...
        def signal_gen(d, i):
            call_count[0] += 1
            if call_count[0] <= 3:
                return Signal(action=Action.LONG, size=0.5, confidence=0.8,
                              instrument="TEST", timestamp=datetime.now(timezone.utc),
                              reasoning="test")
            return Signal(action=Action.FLAT, size=0.0, confidence=1.0,
                          instrument="TEST", timestamp=datetime.now(timezone.utc),
                          reasoning="test")
...
class TestFeaturesCoverage:
    ...
    def test_teleconnection_pdo_dataset_no_var(self):
        ...
        sst = xr.Dataset({"sst": (("latitude", "longitude", "time"),
                                    np.random.rand(5, 5, 24) + 20)},
                         coords={"latitude": np.linspace(25, 55, 5),
    ...
    def test_teleconnection_pdo_no_time_dim(self):
        from pakhi.features.teleconnection import TeleconnectionIndices
        ti = TeleconnectionIndices()
        sst = xr.DataArray(np.random.rand(5, 5) + 20, dims=["latitude", "longitude"],
                           coords={"latitude": np.linspace(25, 55, 5),
                                   "longitude": np.linspace(195, 240, 5)})
    ...
    def test_teleconnection_mjo_dataset_no_var(self):
        ...
        ti = TeleconnectionIndices()
        time_coord = pd.date_range("2020-01-01", periods=24, freq="MS")
        olr = xr.Dataset({"olr": (("latitude", "longitude", "time"),
                                    np.random.rand(5, 5, 24))},
                         coords={"latitude": np.linspace(-10, 10, 5),
    ...
    def test_teleconnection_mjo_std_zero(self):
        ...
        ti = TeleconnectionIndices()
        time_coord = pd.date_range("2020-01-01", periods=24, freq="MS")
        olr = xr.DataArray(np.zeros((3, 3, 24)), dims=["latitude", "longitude", "time"],
                           coords={"latitude": np.linspace(-5, 5, 3),
...
class TestBaseCoverage:
    ...
    def test_take_axis_nonzero(self):
        ...
        time_index = np.array(["2015-01-01", "2016-01-01", "2017-01-01",
                               "2018-01-01", "2019-01-01"], dtype="datetime64[D]")
        X_train, X_val, X_test = train_val_test_split(
            data, train_years=(2015, 2017), val_year=2018, test_year=2019,
            time_index=time_index, axis=1,
        )
...
class TestGaussianCoverage:
    ...
    def test_has_gpytorch_false(self):
        ...
        with patch.dict(sys.modules, {"gpytorch": None}):
            ...
            has_gpt = gaussian._has_gpytorch()
            importlib.reload(gaussian)
...
class TestGridsCoverage:
    ...
    def test_regridder_unsupported_method(self):
        ...
        src = xr.DataArray(np.random.rand(5, 5), dims=["latitude", "longitude"],
                           coords={"latitude": np.arange(5, dtype=float),
                                   "longitude": np.arange(5, dtype=float)})
        dst = xr.DataArray(np.random.rand(3, 3), dims=["latitude", "longitude"],
                           coords={"latitude": np.arange(3, dtype=float),
                                   "longitude": np.arange(3, dtype=float)})
        with pytest.raises(ValueError, match="Unknown"):
            regrid(src, dst, method="invalid_method")

    def test_subset_bbox_wrap(self):
        from pakhi.grids.subset import subset_bbox
        ds = xr.Dataset({"temp": (("lat", "lon"), np.random.rand(10, 20))},
