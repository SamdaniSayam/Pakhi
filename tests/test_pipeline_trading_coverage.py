...
class TestPaperTrader:
    ...
    def _make_signal(self, action, instrument="NG", size=0.1, confidence=0.7):
        ...
        return Signal(
            action=action, size=size, confidence=confidence,
            instrument=instrument, timestamp=datetime.now(timezone.utc),
            reasoning="test",
        )
...
class TestEnsembleSignal:
    ...
    def _sig(self, action, instrument="NG", confidence=0.7):
        ...
        return Signal(
            action=action, size=0.1, confidence=confidence,
            instrument=instrument, timestamp=datetime.now(timezone.utc),
            reasoning="test",
        )
...
class TestAlertManager:
    ...
    def test_check_drought(self):
        ...
        alert = mgr.check_drought({
            "spi_values": spi_vals, "region": "Texas",
        })
        assert alert is not None
...
class TestBacktestEngine:
    ...
    def test_run_basic(self):
        ...
        def gen(data, i):
            return Signal(
                action=Action.LONG, size=0.5, confidence=0.7,
    ...
    def test_run_missing_column(self):
        ...
        with pytest.raises(ValueError, match="Column"):
            engine.run(lambda d, i: Signal(action="FLAT", size=0, confidence=0, instrument="X", timestamp=datetime.now(timezone.utc), reasoning="x"), data)
    ...
    def test_run_short_data(self):
        ...
        result = engine.run(
            lambda d, i: Signal(action=Action.FLAT, size=0, confidence=0, instrument="X", timestamp=datetime.now(), reasoning="x"),
            data,
        )
        assert result.equity_curve.shape[0] == 0
    ...
    def test_walk_forward_short_data(self):
        ...
        results = engine.walk_forward(
            lambda d, i: Signal(action=Action.FLAT, size=0, confidence=0, instrument="X", timestamp=datetime.now(), reasoning="x"),
            data, train_window=30, test_window=20,
        )
        assert len(results) >= 1
    ...
    def test_walk_forward_with_retrain(self):
        ...
        def retrain(train_data):
            return lambda d, i: Signal(
                action=Action.LONG, size=0.5, confidence=0.6,
                instrument="X", timestamp=datetime.now(timezone.utc), reasoning="trained"
            )
        results = engine.walk_forward(
            lambda d, i: Signal(action=Action.FLAT, size=0, confidence=0, instrument="X", timestamp=datetime.now(), reasoning="x"),
            data, train_window=50, test_window=30, retrain_fn=retrain,
        )
        assert len(results) >= 1
    ...
    def test_profit_factor_with_losses(self):
        from pakhi.risk.backtest import BacktestEngine
        pf = BacktestEngine._profit_factor([
            {"pnl": 10}, {"pnl": -5}, {"pnl": 20}, {"pnl": -3}
        ])
    ...
    def test_signal_to_position(self):
        from pakhi.risk.backtest import BacktestEngine
        from pakhi.signals.base import Action, Signal
        sig_l = Signal(action=Action.LONG, size=0.3, confidence=0.7, instrument="X", timestamp=datetime.now(), reasoning="x")
        sig_s = Signal(action=Action.SHORT, size=0.2, confidence=0.6, instrument="X", timestamp=datetime.now(), reasoning="x")
        sig_f = Signal(action=Action.FLAT, size=0.0, confidence=0.0, instrument="X", timestamp=datetime.now(), reasoning="x")
...
class TestTeleconnectionIndices:
    ...
    def _make_sst_data(self):
        ...
        return xr.DataArray(
            sst, dims=["time", "latitude", "longitude"],
            coords={"time": times, "latitude": lats, "longitude": lons},
        )
...
class TestDeterministicPredictor:
    ...
    def test_predict_multi_step_direct(self):
        ...
        with patch.object(DeterministicPredictor, "_clone_model", side_effect=lambda m: MagicMock(predict=m.predict, fit=m.fit)):
            result = dp.predict_multi_step(
                model, np.array([1.0, 2.0, 3.0]), steps=3,
                method="direct", y_train=y_train, X_train=X_train,
            )
    ...
    def test_predict_multi_step_multi_output(self):
        ...
        with patch.object(DeterministicPredictor, "_clone_model", side_effect=lambda m: MagicMock(predict=m.predict, fit=m.fit)):
            result = dp.predict_multi_step(
                model, np.array([1.0, 2.0]), steps=5,
                method="multi_output", y_train=y_train, X_train=X_train,
            )
    ...
    def test_compute_metric_unknown(self):
        ...
        with pytest.raises(ValueError, match="Unknown metric"):
            DeterministicPredictor._compute_metric(
                np.array([0, 1]), np.array([0, 1]), "bad_metric"
            )
...
class TestVerification:
    ...
    def test_brier_skill_score(self):
        ...
        bss = brier_skill_score(
            np.array([0.9, 0.1]), np.array([1.0, 0.0]), climatology_prob=0.5
        )
        assert bss > 0
...
class TestCoordinate:
    ...
    def test_km_to_latlon(self):
        ...
        new_lat, new_lon = km_to_latlon(40.0, -74.0, 100.0, 0.0)
        assert new_lat > 40.0
    ...
    def test_validate_latlon_mismatched_lengths(self):
        from pakhi.grids.coordinate import validate_latlon
        valid, errors = validate_latlon(np.array([1.0, 2.0]), np.array([1.0]))
        assert valid is False
...
class TestModelsInit:
    ...
    def test_direct_imports(self):
        from pakhi.models import BaseModel, PersistenceModel, ClimatologyModel
