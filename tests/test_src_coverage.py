...
class TestYahooCoverage:
    def test_history(self):
        ...
        mock_yf = MagicMock()
        df = pd.DataFrame(
            {"Close": [80.0, 81.0], "Open": [79.0, 80.0],
             "High": [82.0, 83.0], "Low": [78.0, 79.0], "Volume": [1000, 1100]},
    ...
    def test_history_with_start(self):
        ...
        mock_yf = MagicMock()
        df = pd.DataFrame(
            {"Close": [80.0], "Open": [80.0], "High": [81.0],
             "Low": [79.0], "Volume": [1000]},
...
class TestERA5Coverage:
    ...
    def test_fetch_monthly(self):
        from pakhi.src.era5 import ERA5Connector
        conn = ERA5Connector(variables=["temperature_2m"])
        with patch.object(conn, "_download_dataset") as mock_dl, \
...
class TestNOAACoverage:
    ...
    def test_build_url(self):
        from pakhi.src.noaa import GFSConnector, GFS_VARIABLE_MAP
        conn = GFSConnector()
...
class TestMeteostatCoverage:
    ...
    def test_stations_near_fallback(self):
        from pakhi.src.meteostat import MeteostatConnector
        conn = MeteostatConnector()
        with patch.object(conn, "_get", side_effect=ConnectionError("fail")), \
    ...
    def test_stations_near_from_api(self):
        ...
        resp_data = {
            "data": [
                {"id": "ST1", "name": "Test Station", "country": "US",
                 "region": "AL", "latitude": 32.0, "longitude": -88.0,
                 "elevation": 100, "distance": 5.0, "first": "2020", "last": "2024",
...
class TestSatelliteDeep:
    ...
    def test_cloud_motion_with_images(self):
        ...
        with patch.object(conn, "_find_latest_file", side_effect=mock_find), \
...
class TestERA5Deep:
    ...
    def test_fetch_pressure_level(self):
        ...
        with patch.object(conn, "_download_dataset") as mock_dl, \
             patch("pakhi.src.era5.xr.open_dataset") as mock_open:
            mock_dl.return_value = Path(tempfile.mktemp(suffix=".nc"))
            ds = xr.Dataset({"geopotential": (["time", "level", "lat", "lon"], np.random.randn(1, 2, 5, 5))})
            mock_open.return_value = ds
    ...
    def test_download_dataset_error(self):
        ...
        with patch.object(conn, "_get_cds_client", return_value=mock_client):
            with pytest.raises(Exception, match="CDS error"):
    ...
    def test_fetch_with_bbox(self):
        ...
        with patch.object(conn, "_download_dataset") as mock_dl, \
            ...
            mock_open.return_value = ds
            result = conn.fetch("2024-01-01", "2024-01-01", bbox=[50, -125, 24, -66])
            assert conn.area == [50, -125, 24, -66]

    def test_fetch_zarr_import_error(self):
        from pakhi.src.era5 import ERA5Connector
        conn = ERA5Connector()
        with patch.dict("sys.modules", {"zarr": None, "gcsfs": None}):
...
class TestMeteostatDeep:
    ...
    def test_history_multi_partial(self):
        from pakhi.src.meteostat import MeteostatConnector
        conn = MeteostatConnector()
        with patch.object(conn, "history", side_effect=[Exception("fail"), pd.DataFrame({"temp": [10]})]):
            result = conn.history_multi(["ST1", "ST2"], "2024-01-01", "2024-01-01")
...
class TestNOAATrulyDeep:
    ...
    def test_download_retry_all_fail(self):
        ...
        dest.unlink(missing_ok=True)
        with patch.object(conn._session, "get", side_effect=requests.ConnectionError("fail")):
    ...
    def test_open_grib_fallback_engine(self):
        ...
        with patch("pakhi.src.noaa.xr.open_dataset") as mock_open:
            ...
            result = conn._open_grib([Path("test.grib")])
            assert mock_open.call_count == 2

    def test_open_grib_all_fail(self):
        from pakhi.src.noaa import GFSConnector
        conn = GFSConnector()
        with patch("pakhi.src.noaa.xr.open_dataset", side_effect=Exception("all fail")):
            with pytest.raises(RuntimeError, match="No GRIB2 files"):
    ...
    def test_fetch_forecast_with_date(self):
        ...
        with patch.object(conn, "_download_with_retry") as mock_dl, \
             patch.object(conn, "_open_grib", return_value=mock_ds):
            result = conn._fetch_forecast("20240615", "00", 0)
    ...
    def test_fetch_forecast_auto_date(self):
        ...
        conn = GFSConnector(variables=["temperature_2m"])
        mock_ds = xr.Dataset({"t2m": (["time"], [290.0])})
        with patch.object(conn, "_latest_cycle", return_value=("20240615", "00")), \
    ...
    def test_latest(self):
        ...
        with patch.object(conn, "_latest_cycle", return_value=("20240615", "00")), \
            ...
            result = conn.latest(forecast_hour=0)
    ...
    def test_forecast_all_fail(self):
        ...
        with patch.object(conn, "_latest_cycle", return_value=("20240615", "00")), \
            ...
            with pytest.raises(RuntimeError, match="No forecast steps"):
    ...
    def test_archive_all_fail(self):
        from pakhi.src.noaa import GFSConnector
        conn = GFSConnector(variables=["temperature_2m"])
        with patch.object(conn, "_fetch_forecast", side_effect=Exception("fail")):
...
class TestCMEDeep:
    ...
    def test_fetch_settlements_from_api_with_url(self):
        from pakhi.src.cmes import CMEWeatherConnector, CME_PRODUCTS
        conn = CMEWeatherConnector(products=["GAS_DD"])
        with patch.dict(CME_PRODUCTS, {"GAS_DD": {"name": "GAS_DD", "exchange": "CME", "settlement_url": "http://test.com/api"}}):
    ...
    def test_fetch_settlements_from_api_error_fallback(self):
        ...
        conn = CMEWeatherConnector(products=["GAS_DD"])
        with patch.dict(CME_PRODUCTS, {"GAS_DD": {"name": "GAS_DD", "exchange": "CME", "settlement_url": "http://test.com/api"}}):
            with patch.object(conn, "_fetch_json", side_effect=Exception("API fail")):
                result = conn._fetch_settlements_from_api("GAS_DD")
                assert isinstance(result, pd.DataFrame)
    ...
    def test_parse_settlements_nested(self):
        from pakhi.src.cmes import CMEWeatherConnector
        conn = CMEWeatherConnector()
        raw = {"data": [{"date": "2024-01-01", "settlement": 3.5, "change": 0.1, "volume": 1000, "open_interest": 5000}]}
    ...
    def test_latest_settlements(self):
        ...
        with patch.object(conn, "_fetch_settlements_from_api") as mock_fetch:
            mock_fetch.return_value = pd.DataFrame({"product": ["GAS_DD"], "settlement_price": [3.5]})
