...
def test_temporal_features_nans():
    ...
    with patch("pakhi.features.temporal.ema", return_value=np.array([1, 2, 3])):
        with patch("pakhi.features.temporal.rolling_average", return_value=np.array([1, 2, 3])):
...
def test_ensemble_bma_requires_val_return():
    ...
    model = EnsembleForecaster(models=[MockModel()], method="mean")
...
def test_viz_matplotlib_missing():
    ...
    with patch.dict(sys.modules, {"matplotlib": None, "matplotlib.pyplot": None, "plotext": None, "rich": None, "rich.console": None}):
