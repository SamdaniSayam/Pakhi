...
def test_satellite_optical_flow():
    ...
    data = xr.DataArray(
        ...
        dims=["time", "lat", "lon"],
        coords={"time": pd.date_range("2020", periods=3), "lat": np.arange(10), "lon": np.arange(10)}
    )
...
def test_teleconnection_xr():
    ...
    ds = xr.Dataset({"sst": (("time", "lat", "lon"), np.random.rand(10, 10, 10))}, coords={"time": times})
    TeleconnectionIndices.compute_nino34(ds, lat_dim="lat", lon_dim="lon")
    
    ds_slp = xr.Dataset({"slp": (("time", "lat", "lon"), np.random.rand(10, 10, 10))}, coords={"time": times})
    TeleconnectionIndices.compute_nao(ds_slp, lat_dim="lat", lon_dim="lon")
    
    olr = xr.DataArray(np.random.rand(10, 10, 10), coords={"time": times, "lat": np.arange(10), "lon": np.arange(10)}, dims=["time", "lat", "lon"])
...
def test_models_gradient():
    from pakhi.models.gradient import GradientForecaster, _lazy_import_xgboost, _lazy_import_lightgbm
    import sys
    import importlib
...
def test_schedule_job_none():
    ...
    sched._jobs["fake"] = {"next_run": datetime.now(timezone.utc), "interval_hours": 1, "callback": lambda: None}
