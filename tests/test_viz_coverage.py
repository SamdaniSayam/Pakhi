...
class TestDashboardNoRich:
    ...
    def test_display_current_weather_no_rich(self, capsys):
        ...
        with patch.object(dash_mod, "_HAS_RICH", False), \
    ...
    def test_display_current_weather_with_humidity_and_desc(self, capsys):
        ...
        with patch.object(dash_mod, "_HAS_RICH", False), \
            ...
            d.display_current_weather(
                temp=5.0, wind=20.0, pressure=1005.0,
                humidity=80.0, description="Cloudy"
            )
        captured = capsys.readouterr()
    ...
    def test_display_forecast_table_no_rich(self, capsys):
        ...
        with patch.object(dash_mod, "_HAS_RICH", False), \
            ...
            d = TerminalDashboard(use_plotext=False)
            forecast = [
                {"date": "Mon", "temp_high": 30.0, "temp_low": 18.0,
                 "wind": 12.0, "precip_prob": 0.3, "description": "Sunny"},
                {"date": "Tue", "temp_high": 28.0, "temp_low": 16.0,
                 "wind": 8.0, "precip_prob": 0.6, "description": "Rain"},
            ]
    ...
    def test_display_signal_status_no_rich(self, capsys):
        ...
        with patch.object(dash_mod, "_HAS_RICH", False), \
            ...
            d = TerminalDashboard(use_plotext=False)
            signals = [
                {"instrument": "NG", "action": "LONG", "confidence": 0.8,
                 "size": 0.1, "reasoning": "Freeze"},
                {"instrument": "CL", "action": "SHORT", "confidence": 0.6,
                 "size": 0.05, "reasoning": "Demand drop"},
                {"instrument": "HG", "action": "FLAT", "confidence": 0.1,
                 "size": 0.0, "reasoning": "No signal"},
            ]
...
class TestDashboardWithRich:
    ...
    def test_display_current_weather_with_rich(self):
        ...
        d.display_current_weather(
            temp=10.0, wind=5.0, pressure=1010.0,
    ...
    def test_display_signal_status_with_rich(self):
        ...
        if d._console is None:
            pytest.skip("rich not installed")
        signals = [
            {"instrument": "NG", "action": "LONG", "confidence": 0.9,
             "size": 0.2, "reasoning": "Freeze incoming"},
        ]
        d.display_signal_status(signals)
    ...
    def test_display_forecast_table_with_rich(self):
        ...
        d.display_forecast_table([
            {"date": "Mon", "temp_high": 25.0, "temp_low": 15.0,
             "wind": 10.0, "precip_prob": 0.1, "description": "Nice"},
        ])
    ...
    def test_forecast_map_custom_coords(self):
        ...
        try:
            ...
            fig = maps_mod.plot_forecast_map(
                ...
                variable="precipitation", title="Rain Map",
            )
            assert isinstance(fig, matplotlib.figure.Figure)
            plt.close(fig)
        finally:
    ...
    def test_rich_path_with_all_options(self):
        ...
        try:
            ...
            if d._console is not None:
                d.display_current_weather(
                    temp=35.0, wind=25.0, pressure=998.0,
                    humidity=90.0, description="Stormy"
                )
        finally:
...
class TestPlotTerminalChart:
    ...
    def test_plotext_available(self):
        import pakhi.viz.dashboard as dash_mod
        import pakhi.viz.ensemble as ens_mod
